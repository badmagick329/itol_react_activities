        ...
        {filteredJobs.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 text-xs mt-1">
              Use buttons to move jobs between statuses
            </p>
          </div>
        ) : (
          filteredJobs.map((job) => (
            <div
              key={job.id}
              className="bg-gray-800 rounded-lg p-4 border border-gray-700 hover:border-gray-600 transition-colors hover:shadow-lg"
            >
              ...
              <div className="text-xs text-gray-400 mb-2">
                {(job.categories || []).map((category, index) => (
                  <span
                    key={category}
                    className="inline-block px-2 py-1 bg-gray-700 rounded mr-1 mb-1"
                  >
                    {category}
                  </span>
                ))}
...
export default function MultiSelectCategories() {
  const [jobs, setJobs] = useState<Job[]>(() => {
    const savedJobs = localStorage.getItem("multiSelectJobs");
    if (savedJobs) {
      try {
        const parsedJobs = JSON.parse(savedJobs);
        return parsedJobs.map((job: any) => {
          if (job.category && !job.categories) {
            return {
              ...job,
    ...
    return [
      ...
      },
      {
        ...
        categories: ["Send Emails", "Read Emails"],
        status: "stopped",
        dateCreated: new Date(
  ...
  useEffect(() => {
    localStorage.setItem("multiSelectJobs", JSON.stringify(jobs));
  }, [jobs]);
  ...
  const addSampleJob = () => {
    const sampleJobs = [
      {
        ...
        categories: ["Read Emails"] as JobCategory[],
        status: "start" as JobStatus,
      },
      {
        title: "Product Data Scraping",
        categories: ["Web Parsing"] as JobCategory[],
        status: "start" as JobStatus,
      },
      {
        title: "Welcome Email Campaign",
        categories: ["Send Emails"] as JobCategory[],
        status: "start" as JobStatus,
      },
      {
        title: "Social Media Data Mining",
        categories: ["Web Parsing", "Read Emails"] as JobCategory[],
        status: "start" as JobStatus,
      },
      {
        title: "Support Ticket Processing",
        categories: ["Read Emails", "Send Emails"] as JobCategory[],
        status: "start" as JobStatus,
      },
      {
    ...
    addNewJob(randomJob.title, randomJob.categories, randomJob.status);
  };
...
function MultiSelectJobForm({
  ...
  const handleCategoryToggle = (category: JobCategory) => {
    setFormData((prev) => {
      ...
      let newCategories: JobCategory[];

      if (isSelected) {
        newCategories = prev.categories.filter((c) => c !== category);
      } else {
        if (prev.categories.length >= MAX_CATEGORIES) {
          setValidationError(`Maximum ${MAX_CATEGORIES} categories allowed`);
          return prev;
        }
        newCategories = [...prev.categories, category];
      }

      if (validationError && newCategories.length > 0) {
        setValidationError("");
      }
  ...
  const validateForm = () => {
    ...
    if (formData.categories.length === 0) {
      setValidationError("Please select at least one job category");
      return false;
    }
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <label className="block text-sm font-medium text-gray-300">
            Job Categories * (Select up to {MAX_CATEGORIES})
          </label>
          {formData.categories.length > 0 && (
            ...
            >
              Clear All ({formData.categories.length})
            </button>
          )}
        </div>
        ...
        <div className="mb-3">
          <input
            ...
            className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-white placeholder-gray-400 text-sm"
          />
        </div>
          ...
          {filteredCategories.map((category) => {
            ...
            return (
              <button
                ...
                disabled={
                  !isSelected && formData.categories.length >= MAX_CATEGORIES
                }
              >
                {isSelected ? "✓ " : ""}
                {category}
        ...
        {formData.categories.length > 0 && (
          <div className="bg-gray-700/50 rounded-lg p-3 border border-gray-600">
            <p className="text-sm text-gray-300 mb-2">
              Selected Categories ({formData.categories.length}/{MAX_CATEGORIES}
              ):
            </p>
